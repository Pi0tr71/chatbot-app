...
from models.openai import (
    ...
    openai_chat_completion_title,
)
from models.nebius import NebiusModels, NebiusProvider, nebius_chat_completion
from models.sambanova import sambanova_chat_completion

from ui.helpers import split_response, get_next_chat_name, check_existing
from utils.costs import cost_cal
from utils.history import save_history
from utils.costs import update_costs


class Models(OpenAIModels, NebiusModels):
    pass
...
def render_chat_interface(config: Settings, enc):
    # CHAT INTERFACE
    st.title("Chatbot")
    ...
    for msg in messages:
        with st.chat_message(msg["role"]):
            if msg["role"] == "assistant":
                ...
                st.markdown(
                    f"<div style='text-align: right;'><i>Model: {msg["model"]}: Used tokens: {msg["tokens_used"]}, Price: {msg["cost"]:.6f}$</i></div>",
                    unsafe_allow_html=True,
                )
            else:
                st.write(msg["content"])
                st.markdown(
                    f"<div style='text-align: right;'><i>Used tokens: {msg["tokens_used"]}, Price: {msg["cost"]:.6f}$</i></div>",
                    unsafe_allow_html=True,
                )
    ...
    if user_input:
        ...
        with st.chat_message("user"):
            st.write(user_input)
            st.markdown(
                f"<div style='text-align: right;'><i>Used tokens: {input_tokens}, Cost: {input_cost:.6f}$</i></div>",
                unsafe_allow_html=True,
            )
        ...
        with st.spinner("Waiting for response..."):
            try:
                ...
                if config["model"] in [
                    ...
                    response, input_tokens, output_tokens = openai_chat_completion(
                        messages,
                        config["model"],
                    ...
                    nebuis_provider = NebiusProvider(config["api_keys"]["nebius"])
                    response = nebuis_provider.chat_completion(
                        messages, config["model"], nebuis_provider, context_length * 2
                    )
                    output_tokens = len(enc.encode(response))

                elif config["model"] in ["DeepSeek-R1-Distill-70B", "Tulu-3-405B"]:
                    response = sambanova_chat_completion(
                        messages,
                        config["model"],
                        config["api_keys"]["sambanova"],
                        context_length * 2,
                    )
                    output_tokens = len(enc.encode(response))
                ...
                with st.chat_message("assistant"):
                    ...
                    st.markdown(
                        f"<div style='text-align: right;'><i>Model: {config["model"]}: Used tokens: {output_tokens}, Cost: {output_cost:.6f}$</i></div>",
                        unsafe_allow_html=True,
                    )
        ...
        update_history_on_sidebart(st.session_state, messages)
        ...
        if st.session_state.current_chat:
            ...
            try:
                chat_name = openai_chat_completion_title(
                    messages, config["api_keys"]["openai"]
                )
