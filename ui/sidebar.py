...
from ui.helpers import get_next_chat_name
...
def render_side(config):
    ...
    if st.session_state.current_chat:
        new_chat_name = st.sidebar.text_input(
            "Chat name", st.session_state.current_chat
        )
        if new_chat_name and new_chat_name != st.session_state.current_chat:
            if new_chat_name in st.session_state.history:
                ...
                st.session_state.history[new_chat_name] = st.session_state.history.pop(
    ...
    with st.sidebar.expander("Context settings"):
        st.session_state.context_length = st.slider(
            "How many past messages to include (past user messages)",
            min_value=0,
            max_value=5,
            value=0,
        )
    ...
    with st.sidebar.expander("API keys"):
        openai_key = st.text_input(
            "OpenAI API Key", value=config["api_keys"]["openai"], type="password"
        )
        nebius_key = st.text_input(
            "Nebius API Key", value=config["api_keys"]["nebius"], type="password"
        )
        sambanova_key = st.text_input(
            "Sambanova API Key", value=config["api_keys"]["sambanova"], type="password"
        )
    ...
    def render_chat_list():
        ...
        if chat_names:
            ...
            for chat in chat_names:
                last_active = st.session_state.history[chat].get(
                    "last_active", "No data"
                )
            ...
            for chat, last_active in chat_last_active:
                ...
                with col1:
                    if st.button(
                        f"{chat} ({last_active.strftime("%d-%m-%Y %H:%M")})",
                        key=f"chat_{chat}",
                    ):
